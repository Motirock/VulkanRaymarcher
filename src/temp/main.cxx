...
private:
    ...
    int CHUNK_GRID_X_SIZE = 4, CHUNK_GRID_Y_SIZE = 4, CHUNK_GRID_Z_SIZE = 4;
    int WORLD_X_SIZE = CHUNK_GRID_X_SIZE*CHUNK_SIZE, WORLD_Y_SIZE = CHUNK_GRID_Y_SIZE*CHUNK_SIZE, WORLD_Z_SIZE = CHUNK_GRID_Z_SIZE*CHUNK_SIZE;
    Chunk **chunkGrid = new Chunk*[CHUNK_GRID_X_SIZE*CHUNK_GRID_Y_SIZE*CHUNK_GRID_Z_SIZE];
    ...
    void loadBlockTypes(std::string fileName) {
        std::ifstream blocksFile("data/"+fileName+".json", std::ifstream::binary);
        Json::Value blocks;
        blocksFile >> blocks;
    ...
    void saveChunk(Chunk *sourceChunk, std::string fileName) {
        std::ofstream fout;
        fout.open("data/"+fileName+".bin", std::ios::binary | std::ios::out);

        fout.write((char*) &sourceChunk->blockGrid, sizeof(sourceChunk->blockGrid));

        fout.close();
    }

    void loadChunk(Chunk *destinationChunk, std::string fileName) {
        std::ifstream fin;
        fin.open("data/"+fileName+".bin", std::ios::binary | std::ios::in);
        
        fin.get((char*) &destinationChunk->blockGrid, sizeof(destinationChunk->blockGrid));

        fin.close();
        destinationChunk->blocksChanged = true;
    }

    void saveWorld(std::string worldName) {
        Json::Value world;

        std::cout << "Saving world " << worldName << '\n';

        world["xSize"] = CHUNK_GRID_X_SIZE;
        world["ySize"] = CHUNK_GRID_Y_SIZE;
        world["zSize"] = CHUNK_GRID_Z_SIZE;
        ...
        std::unique_ptr<Json::StreamWriter> writer(builder.newStreamWriter());
        std::ofstream outputFileStream("data/worlds/"+worldName+"/"+worldName+".json");
        ...
        for (int i = 0; i < CHUNK_GRID_X_SIZE; i++) {
            for (int j = 0; j < CHUNK_GRID_Y_SIZE; j++) {
                for (int k = 0; k < CHUNK_GRID_Z_SIZE; k++) {
                    std::ofstream fout;
                    std::string fileName = std::to_string(i)+"_"+std::to_string(j)+"_"+std::to_string(k);
                    fout.open("data/worlds/"+worldName+"/chunks/"+fileName+".bin", std::ios::binary | std::ios::out);

                    fout.write((char*) &getChunk(i, j, k)->blockGrid, sizeof(getChunk(i, j, k)->blockGrid));
    ...
    void loadWorld(std::string worldName) {
        std::ifstream worldFile("data/worlds/"+worldName+"/"+worldName+".json", std::ifstream::binary);
        Json::Value world;
        worldFile >> world;

        std::cout << "Loading world " << world["name"] << '\n';

        CHUNK_GRID_X_SIZE = world["xSize"].asInt();
        CHUNK_GRID_Y_SIZE = world["ySize"].asInt();
        CHUNK_GRID_Z_SIZE = world["zSize"].asInt();

        WORLD_X_SIZE = CHUNK_GRID_X_SIZE*CHUNK_SIZE;
        WORLD_Y_SIZE = CHUNK_GRID_Y_SIZE*CHUNK_SIZE;
        WORLD_Z_SIZE = CHUNK_GRID_Z_SIZE*CHUNK_SIZE;

        delete [] chunkGrid;
        //delete chunkGrid;
        chunkGrid = new Chunk*[CHUNK_GRID_X_SIZE*CHUNK_GRID_Y_SIZE*CHUNK_GRID_Z_SIZE];
       // throw std::runtime_error("asdf");
        for (int i = 0; i < CHUNK_GRID_X_SIZE; i++) {
            for (int j = 0; j < CHUNK_GRID_Y_SIZE; j++) {
                for (int k = 0; k < CHUNK_GRID_Z_SIZE; k++) {
                    Chunk *destinationChunk = new Chunk(i*CHUNK_SIZE, j*CHUNK_SIZE, k*CHUNK_SIZE);
                    setChunk(i, j, k, destinationChunk);
                    destinationChunk->geometry.device = device;

                    std::ifstream fin;
                    std::string fileName = std::to_string(i)+"_"+std::to_string(j)+"_"+std::to_string(k);
                    fin.open("data/worlds/"+worldName+"/chunks/"+fileName+".bin", std::ios::binary | std::ios::in);
    ...
    void input() {
        ...
        if (glfwGetKey(window, GLFW_KEY_R) == GLFW_PRESS) {
            int radius = 10;
            int playerX = static_cast<int>(cameraPosition.x);
            int playerY = static_cast<int>(cameraPosition.y);
            int playerZ = static_cast<int>(cameraPosition.z);

            for (int x = playerX - radius; x <= playerX + radius; x++) {
                for (int y = playerY - radius; y <= playerY + radius; y++) {
                    for (int z = playerZ - radius; z <= playerZ + radius; z++) {
                        if (glm::distance(glm::vec3(x, y, z), cameraPosition) > radius ||!(x >= 0 && x < CHUNK_GRID_X_SIZE * CHUNK_SIZE && y >= 0 && y < CHUNK_GRID_Y_SIZE * CHUNK_SIZE && z >= 0 && z < CHUNK_GRID_Z_SIZE * CHUNK_SIZE))
                            continue;
                        setBlock(x, y, z, 0);
                    }
                }
    ...
    void recordCommandBuffer(VkCommandBuffer commandBuffer, uint32_t imageIndex) {
        ...
        clearValues[0].color = {{0.2f, 0.25f, 0.7f, 1.0f}};
            ...
            for (int i = 0; i < CHUNK_GRID_X_SIZE; i++) {
                for (int j = 0; j < CHUNK_GRID_Y_SIZE; j++) {
                    for (int k = 0; k < CHUNK_GRID_Z_SIZE; k++) {
                        if (getChunk(i, j, k)->blocksChanged) {
                            Chunk *chunk = getChunk(i, j, k);
                            chunk->graphicsChanged = true;
                            if (i < CHUNK_GRID_X_SIZE-1) {
                                borderingChunk = getChunk(i+1, j, k);
                                borderingChunk->graphicsChanged = true;
                                if (!borderingChunk->blocksChanged)
                                    chunk->generateMesh(POSITIVE_X, borderingChunk, blockTypes);
                                borderingChunk->generateMesh(NEGATIVE_X, chunk, blockTypes);
                            }
                            else {
                                chunk->generateMesh(POSITIVE_X, NULL, blockTypes);
                            }
                            if (i > 0) {
                                borderingChunk = getChunk(i-1, j, k);
                                borderingChunk->graphicsChanged = true;
                                if (!borderingChunk->blocksChanged)
                                    chunk->generateMesh(NEGATIVE_X, borderingChunk, blockTypes);
                                borderingChunk->generateMesh(POSITIVE_X, chunk, blockTypes);
                            }
                            else {
                                chunk->generateMesh(NEGATIVE_X, NULL, blockTypes);
                            }
                            if (j < CHUNK_GRID_Y_SIZE-1) {
                                borderingChunk = getChunk(i, j+1, k);
                                borderingChunk->graphicsChanged = true;
                                if (!borderingChunk->blocksChanged)
                                    chunk->generateMesh(POSITIVE_Y, borderingChunk, blockTypes);
                                borderingChunk->generateMesh(NEGATIVE_Y, chunk, blockTypes);
                            }
                            else {
                                chunk->generateMesh(POSITIVE_Y, NULL, blockTypes);
                            }
                            if (j > 0) {
                                borderingChunk = getChunk(i, j-1, k);
                                borderingChunk->graphicsChanged = true;
                                if (!borderingChunk->blocksChanged)
                                    chunk->generateMesh(NEGATIVE_Y, borderingChunk, blockTypes);
                                borderingChunk->generateMesh(POSITIVE_Y, chunk, blockTypes);
                            }
                            else {
                                chunk->generateMesh(NEGATIVE_Y, NULL, blockTypes);
                            }
                            if (k < CHUNK_GRID_Z_SIZE-1) {
                                borderingChunk = getChunk(i, j, k+1);
                                borderingChunk->graphicsChanged = true;
                                if (!borderingChunk->blocksChanged)
                                    chunk->generateMesh(POSITIVE_Z, borderingChunk, blockTypes);
                                borderingChunk->generateMesh(NEGATIVE_Z, chunk, blockTypes);
                            }
                            else {
                                chunk->generateMesh(POSITIVE_Z, NULL, blockTypes);
                            }
                            if (k > 0) {
                                borderingChunk = getChunk(i, j, k-1);
                                borderingChunk->graphicsChanged = true;
                                if (!borderingChunk->blocksChanged)
                                    chunk->generateMesh(NEGATIVE_Z, borderingChunk, blockTypes);
                                borderingChunk->generateMesh(POSITIVE_Z, chunk, blockTypes);
                            }
                            else {
                                chunk->generateMesh(NEGATIVE_Z, NULL, blockTypes);
                            }

                            int totalFaces = 0;
                            for (int i = 0; i < NUMBER_OF_ORIENTATIONS-1; i++) {
                                totalFaces += chunk->faces[i].size();
                            }
                            std::cout << "Done generating mesh for chunk at " << chunk->x << " " << chunk->y << " " << chunk->z << " with " << totalFaces << " total faces" << std::endl;
                        }
                    }
                }
            }
            ...
            for (int i = 0; i < CHUNK_GRID_X_SIZE; i++) {
                for (int j = 0; j < CHUNK_GRID_Y_SIZE; j++) {
                    for (int k = 0; k < CHUNK_GRID_Z_SIZE; k++) {
                        ...
                        getChunk(i, j, k)->graphicsChanged = false;
                    }
                }
            }
        }
