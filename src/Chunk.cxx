    ...
    for (int z = 0; z < CHUNK_SIZE; z++) {
        for (int y = 0; y < CHUNK_SIZE; y++) {
            for (int x = 0; x < CHUNK_SIZE; x++) {
                //Reseting values
                values[valueIndex] = pow((z+position.z)/WORLD_DIMENSIONS_VOXELS.z, 1.5) < pow((x+position.x)/WORLD_DIMENSIONS_VOXELS.x, 2)+pow((y+position.y)/WORLD_DIMENSIONS_VOXELS.y, 2) ? 4 : 0;

                float r = 0.0f;
                r = noise.octave2D_01((x+position.x)/500.0f, (y+position.y)/500.0f, 6)*CHUNK_SIZE*WORLD_DIMENSIONS_CHUNKS.z;

                bool isSolid = r >= (float) z+position.z;
                if (isSolid && r >= (float) z+position.z+3)
                    values[valueIndex] = 3;
                else if (isSolid && r >= (float) z+position.z+1)
                    values[valueIndex] = 2;
                else if (isSolid)
                    values[valueIndex] = 1;
...
void Chunk::createOctree() {
    ...
    for (int nodeIndex = 0; nodeIndex < nodeSlotsUsed; nodeIndex++) {
        ...
        if (tempOctree.nodes[nodeIndex].value == 0)
            ...
            tempOctree.nodes[nodeIndex].color = glm::vec4(0.0f, 0.4f+(rand()%10)/100.0f, 0.0f, 1.0f);
