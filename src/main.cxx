...
/* Quick Use
glslc shaders/shader.vert -o shaders/vert.spv && glslc shaders/shader.frag -o shaders/frag.spv && glslc shaders/raymarcher.comp -o shaders/raymarcher.spv && g++ -c -std=c++17 -O3 src/*.cxx -Iinclude && g++ *.o -o bin/main -Llib -lglfw -lvulkan -ldl -lpthread -lX11 -lXxf86vm -lXrandr -lXi && ./bin/main
*/
...
class VoxelRaymarcherApplication {
public:
    void run() {
        initWindow();
        initVulkan();
...
private:
    ...
    VkBuffer regionStagingBuffer;
    VkDeviceMemory regionStagingBufferMemory;
    GPUOctree *regionStagingBufferMapped;
    VkBuffer *octreeStorageBuffers;
    VkDeviceMemory *octreeStorageBuffersMemory;
    ...
    //UBO data
    glm::vec3 cameraPosition = glm::vec3(WORLD_DIMENSIONS_VOXELS.x*0.2001f, WORLD_DIMENSIONS_VOXELS.y*0.2f, WORLD_DIMENSIONS_VOXELS.z*0.9f);//glm::vec3(WORLD_DIMENSIONS.x/2.0f, WORLD_DIMENSIONS.y/2.0f, WORLD_DIMENSIONS.z/2.0f);
    glm::vec3 viewDirection = glm::normalize(glm::vec3(1.0f, 1.0f, 1.0f));
    //Degrees, xy plane, xz plane
    glm::vec2 viewAngles = glm::vec2(-45, -45);
    float FOV = 60.0f;
    ...
    void input() {
        ...
        if (glfwGetKey(window, GLFW_KEY_SPACE) == GLFW_PRESS)
            cameraPosition.z += speed/(float) TPS;
        ...
        while (!glfwWindowShouldClose(window)) {
            ...
            if (time*TPS >= ticks) {
                ...
                std::cout << "Frame time: " << (float) averageFrameTime <<
                    ...
                    << "Voxel memory:         " << sizeof(GPUOctree)*WORLD_DIMENSIONS_CHUNKS.x*WORLD_DIMENSIONS_CHUNKS.y*WORLD_DIMENSIONS_CHUNKS.z/1'000'000'000.0 << " gb\n"
    ...
    void createInstance() {
        ...
        appInfo.pApplicationName = "Raymarcher";
    ...
    void createComputePipeline() {
        auto computeShaderCode = readFile("shaders/raymarcher.spv");
    ...
    void createStorageImage() {
        ...
        storageImageView = createImageView(storageImage, VK_FORMAT_R8G8B8A8_UNORM, VK_IMAGE_ASPECT_COLOR_BIT, 1);
    ...
    void createVertexBuffer() { 
        vertices.clear();
        vertices.push_back({{-3.0f, -1.0f}, {1.0f, 0.0f, 0.0f}});
        ...
        copyBuffer(stagingBuffer, vertexBuffer, 0, 0, bufferSize);
    ...
    void createOctreeStorageBuffer() {
        ...
        for (int i = 0; i < WORLD_DIMENSIONS_VOXELS.x/CHUNK_SIZE; i++) {
            for (int j = 0; j < WORLD_DIMENSIONS_VOXELS.y/CHUNK_SIZE; j++) {
                for (int k = 0; k < WORLD_DIMENSIONS_VOXELS.z/CHUNK_SIZE; k++) {
                    ...
                    // std::cout << "Chunk " << i << " " << j << " " << k << " created\n" << bufferIndex << ' ' << i + j*WORLD_DIMENSIONS_CHUNKS.x + k*WORLD_DIMENSIONS_CHUNKS.x*WORLD_DIMENSIONS_CHUNKS.y;

                    // memcpy(regionStagingBufferMapped, &chunk.octree, (size_t) bufferSize);

                    // createBuffer(bufferSize, VK_BUFFER_USAGE_TRANSFER_DST_BIT | VK_BUFFER_USAGE_STORAGE_BUFFER_BIT, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, octreeStorageBuffers[bufferIndex], octreeStorageBuffersMemory[bufferIndex]);

                    // copyBuffer(regionStagingBuffer, octreeStorageBuffers[bufferIndex], bufferSize);
        ...
        //Move to cleanUp()
        vkUnmapMemory(device, regionStagingBufferMemory);

        vkDestroyBuffer(device, regionStagingBuffer, nullptr);
        vkFreeMemory(device, regionStagingBufferMemory, nullptr);
    }
    ...
    void recordComputeCommandBuffer(VkCommandBuffer commandBuffer) {
        ...
        if (STORAGE_IMAGE_SIZE % 4 != 0) {
            throw std::runtime_error("STORAGE_IMAGE_SIZE must be a multiple of 4!");
        }
        vkCmdDispatch(commandBuffer, STORAGE_IMAGE_SIZE/4, STORAGE_IMAGE_SIZE/4, 1);
    ...
    void updateUniformBuffer(uint32_t currentImage) {
        ...
        float currentTime = ticks / 60.0f;

        //Calculate the angle of the light direction based on the current time
        float angle = glm::radians((currentTime / 10.0f) * 360.0f);

        //Calculate the x and z components of the light direction
        float lightDirectionX = glm::cos(angle);
        float lightDirectionY = glm::sin(angle);

        //Set the light direction
        ubo.lightDirection = glm::normalize(glm::vec3(lightDirectionX, lightDirectionY, 1.0f));
        
        glm::vec3 clearSkyColor = glm::vec3(0x87/255.0f, 0xCE/255.0f, 0xEB/255.0f);
        ubo.skyColor = clearSkyColor*ubo.lightDirection.z;
        if (ubo.lightDirection.z < 0.0f)
            ubo.skyColor = glm::vec3(0.0f);
            ...
            for (int i = 0; i < WORLD_DIMENSIONS_VOXELS.x/CHUNK_SIZE; i++) {
                for (int j = 0; j < WORLD_DIMENSIONS_VOXELS.y/CHUNK_SIZE; j++) {
                    for (int k = 0; k < WORLD_DIMENSIONS_VOXELS.z/CHUNK_SIZE; k++) {
                        ...
                        std::cout << "Chunk " << i << " " << j << " " << k << " created" << std::endl;
                    }
                }
            }
...
int main() {
    VoxelRaymarcherApplication app;
